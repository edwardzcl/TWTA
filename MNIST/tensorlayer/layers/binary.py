...
def _ternary_operation1(x, scale):
    """
    Ternary operation use threshold computed with weights.
    """
    g = tf.get_default_graph()
    with g.gradient_override_map({"Sign": "Identity"}):
        #threshold = _compute_threshold(x)
        x = tf.sign(tf.add(tf.sign(tf.add(x, scale)), tf.sign(tf.add(x, -scale))))
        return x

def _ternary_operation2(x):
    """
    Ternary operation use threshold computed with weights.
    """
    g = tf.get_default_graph()
    with g.gradient_override_map({"Sign": "Identity"}):
        #threshold = _compute_threshold(x)
        x = tf.sign(tf.add(tf.sign(tf.add(x, 0.5)), tf.sign(tf.add(x, -0.5))))
        return x
...
class TernaryLayer(Layer):
    """The :class:`TernaryLayer` class is for quantizing the layer outputs to -1 or 1 while inferencing.
    ...
    def __init__(
        ...
        super(TernaryLayer, self).__init__(prev_layer=prev_layer, name=name)

        self.inputs = prev_layer.outputs

        logging.info("TernaryLayer  %s" % (self.name))
        ...
        with tf.variable_scope(name):
            ...
            #        name='s', shape=(1), initializer=tf.constant_initializer(value=0.0), dtype=LayersConfig.tf_dtype, **s_init_args
            #)
            #self.outputs = _ternary_operation1(self.inputs, scale)
            self.outputs = _ternary_operation2(self.inputs)
...
class SignLayer(Layer):
    ...
    def __init__(
        ...
        with tf.variable_scope(name):
            ...
            self.inputs = tf.clip_by_value(self.inputs, -2, 2, name=name)
            self.outputs = _quantize_active(_cabs(self.inputs), bitA)
